...
import { BookOpen, MapPin, Mail, MessageSquare, UserPlus, ExternalLink } from "lucide-react"
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="mb-10">
          <div className="flex flex-col md:flex-row gap-8 items-start">
            <Avatar className="w-32 h-32">
              <AvatarImage src={`/placeholder.svg?height=128&width=128&query=professional academic portrait`} />
              <AvatarFallback>
                {researcher.name
                  .split(" ")
                  .map((n) => n[0])
            ...
            <div className="flex-1">
              <h1 className="text-4xl font-semibold mb-2">{researcher.name}</h1>
              <p className="text-lg text-muted-foreground mb-1">{researcher.title}</p>
              <p className="text-base text-muted-foreground mb-4">{researcher.institution}</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          <div className="border border-border rounded-lg p-4 text-center">
            <div className="text-3xl font-semibold text-primary mb-1">{researcher.hIndex}</div>
            <div className="text-xs text-muted-foreground">H-Index</div>
          </div>
          <div className="border border-border rounded-lg p-4 text-center">
            <div className="text-3xl font-semibold text-primary mb-1">{researcher.publications}</div>
            <div className="text-xs text-muted-foreground">Publications</div>
          </div>
          <div className="border border-border rounded-lg p-4 text-center">
            <div className="text-3xl font-semibold text-primary mb-1">{researcher.citations}</div>
            <div className="text-xs text-muted-foreground">Citations</div>
          </div>
          <div className="border border-border rounded-lg p-4 text-center">
            <div className="text-3xl font-semibold text-primary mb-1">{researcher.students}</div>
        ...
        <Tabs defaultValue="about" className="space-y-6">
          ...
          <TabsContent value="about" className="space-y-6">
            <div className="border border-border rounded-lg p-6">
              ...
              <p className="text-muted-foreground leading-relaxed text-sm">{researcher.bio}</p>
            </div>

            <div className="border border-border rounded-lg p-6">
              <h3 className="font-semibold text-foreground mb-4">Research Interests</h3>
              <div className="flex flex-wrap gap-2">
                {["Deep Learning", "Computer Vision", "Neural Networks", "Transfer Learning", "AI Ethics"].map(
                  (interest) => (
          ...
          <TabsContent value="students" className="space-y-4">
            <div className="border border-border rounded-lg p-6">
              <h3 className="font-semibold text-foreground mb-4">Current Students</h3>
          ...
          <TabsContent value="projects">
            <div className="border border-border rounded-lg p-6">
              <h3 className="font-semibold text-foreground mb-2">Active Research Projects</h3>
