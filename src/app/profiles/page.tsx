    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="mb-10">
          ...
          <p className="text-muted-foreground">Discover and connect with leading researchers worldwide</p>
        </div>

        <div className="flex flex-col md:flex-row gap-4 mb-8">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-muted-foreground" />
            <Input placeholder="Search researchers..." className="pl-10 text-sm" />
          </div>
          <Select>
            ...
            >
              <div className="flex items-start gap-4">
                <Avatar className="w-16 h-16">
                  <AvatarImage src={`/placeholder.svg?height=64&width=64&query=professional academic portrait`} />
                  <AvatarFallback>
                    {researcher.name
                      .split(" ")
                      .map((n) => n[0])
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <h3 className="font-semibold text-foreground">
                        <Link href={`/profile/${researcher.id}`} className="hover:text-primary transition-colors">
                          {researcher.name}
                        </Link>
                      </h3>
                  ...
                  <p className="text-sm text-muted-foreground mb-3">{researcher.bio}</p>
                  <div className="flex items-center gap-6 text-sm text-muted-foreground mb-4">
                    <span>
                      H-Index: <span className="font-medium text-foreground">{researcher.hIndex}</span>
                    </span>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button size="sm" variant="ghost" asChild>
                      <Link href={`/profile/${researcher.id}`}>View Profile</Link>
                    </Button>
                  </div>
                </div>
              </div>
